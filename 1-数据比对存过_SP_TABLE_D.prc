create or replace procedure SP_TABLE_D(tab_name       in varchar2, --比对表名
...
begin
  ...
  select count(1)
   ...
   where 1 = 1
     ...
     and a.TABLE_NAME = upper(tab_name) || '_D';

  v_sql_ddl := 'create table ' || user_name_out || '.' || upper(tab_name) ||
               '_D' || ' as select * from ' || user_name_from || '.' ||
               upper(tab_name) || ' where 1=2';
